...
print("▶ join channel gw dulu mass")
os.system("https://t.me/@mrzxleak")
if "linux" in sys.platform.lower():
	os.system(
			"clear"
	)
...
def title_loop():
    ...
    if os.name == "nt":
        while True:
            os.system(f'title TikTok Viewbot by @MrZxRX ^| success: {success} fails: {fails} reqs: {reqs} rps: {rps} rpm: {rpm}')
            time.sleep(0.1)

if __name__ == "__main__":
    os.system("cls" if os.name == "nt" else "clear"); os.system("title TikTok Viewbot by @MrZxRX" if os.name == "nt" else "")
    txt = """\n\n ╦  ╦╦╔═╗╦ ╦╔╗ ╔═╗╔╦╗\n ╚╗╔╝║║╣ ║║║╠╩╗║ ║ ║ \n  ╚╝ ╩╚═╝╚╩╝╚═╝╚═╝ ╩\n- Youtube: OnlyWan~\n- Script : Viewer tiktok\n- Creator: @MrZxRX"""
    print(
        Colorate.Vertical(
